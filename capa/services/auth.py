...
def create_user( db: Session, user: CreateUserRequest ):
  ...
  if usr_email:
    raise HTTPException( status_code=status.HTTP_400_BAD_REQUEST, detail='Email registrado' )
  ...
  db.commit()
  db.refresh( db_user )  
  token = create_access_token( db_user.usr_email, db_user.usr_id, timedelta(minutes=15) )
  # send_email(user.usr_email, token)



def authenticate_user( usr_email: str, usr_password: str, db: Session, ):
  user = db.query( UserModel ).filter( UserModel.usr_email == usr_email ).first()
  
  if not user:
    raise HTTPException( status_code=status.HTTP_404_NOT_FOUND, detail='Usuario o contraseña incorrecto' )  
  if not verify_password( usr_password, user.usr_password ):
    raise HTTPException( status_code=status.HTTP_400_BAD_REQUEST, detail='Usuario o contraseña incorrecto' )
...
def get_current_user(token: Annotated[str, Depends(oauth2_bearer)]):
  try:
    ...
    if usr_email is None or usr_id is None:
      raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail='Usuario no autorizado gcu')
    return {'usr_email': usr_email, 'usr_id': usr_id}
  except JWTError:
    raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail='Usuario no autorizado gcu jwtError')
